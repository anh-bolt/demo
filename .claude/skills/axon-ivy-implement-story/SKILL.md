...
Before marking any part of a story as complete, every produced artifact MUST comply with [.claude/rules/axon-ivy-coding.md](../../rules/axon-ivy-coding.md). The per-artifact skills already encode those rules — do not override them to "move faster":

- Java → one public type per file, no wildcard imports, Javadoc, role-based package, SLF4J parameterized logging, no PII, exceptions preserve cause.
- Processes → verb-noun names, layered (business / detail / technical), Script logging parameterized.
- Dialogs → use-case names, no hardcoded UI strings (everything via CMS).
- CMS → folder purpose (`/messages`, `/errors`, `/ui`, `/templates`, `/email`), placeholders not concatenation, enum/constant access from Java.
- Config → grouped namespaces, secrets declared as `Password`, synthetic test users only.

**Gate**: after implementing each `### Part X:`, the next step runs the relevant `*-verify` skill. That skill WILL fail if coding-rule checks (verify-story Phase 5, process-verify §14, cms-verify §11–§13) don't pass — fix violations before proceeding.
